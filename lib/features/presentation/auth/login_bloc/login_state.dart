...
class SendSignupOtpLoading extends LogInState {
  const SendSignupOtpLoading();
}

class SendSignupOtpFailureState extends LogInState {
  final String errorMessage;
  const SendSignupOtpFailureState(this.errorMessage);
}

class NavigateToVerifyOtpForSignup extends LogInState {
  final SendOtpEntity sendOtpEntity;
  final SignupDraftData signupDraft;

  const NavigateToVerifyOtpForSignup({
    required this.sendOtpEntity,
    required this.signupDraft,
  });
}

class RegisterPatientLoading extends LogInState {
  const RegisterPatientLoading();
}

class RegisterPatientFailureState extends LogInState {
  final String errorMessage;
  const RegisterPatientFailureState(this.errorMessage);
}
