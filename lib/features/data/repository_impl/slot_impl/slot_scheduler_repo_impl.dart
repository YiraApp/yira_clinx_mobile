...
class SchedulerRepositoryImpl implements SchedulerRepository {
    ...
    try {
      ...
      for (final s in slotObjects) {
        if (s is SlotEntity) {
          ...
          final start = DateFormat('HH:mm').format(parseTime(s.startTime));
          final end = DateFormat('HH:mm').format(parseTime(s.endTime));
      ...
      debugPrint("SlotRepo: Deploying ${slotsPayload.length} slots for doctor=$doctorId, hospital=$hospitalId, date=$targetDate");
  ...
  Future<bool> deployTimeSchedule(List<TimeSlot> slots, {required String targetDate, required bool isSingleDay}) async {
    return true; // Already deployed via deploySchedule
  }
