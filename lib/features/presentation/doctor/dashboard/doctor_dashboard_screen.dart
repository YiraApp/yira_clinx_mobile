// presentation/doctor/dashboard/doctor_dashboard_screen.dart
...
class _DoctorDashboardScreenState extends State<DoctorDashboardScreen> {
                      ...
                      SliverPadding(
                        ...
                        sliver: SliverList(
                          delegate: SliverChildListDelegate([
                            ...
                            _buildWelcomeCard(context, isDark, primaryColor, isTabletDevice),
                            const SizedBox(height: fieldSpace),
                            _buildMetricsGrid(context, state, primaryColor, isTabletDevice),
                            const SizedBox(height: fieldSpace),
                            _buildSectionHeader(
                              context,
                              "Today's Schedule",
                          ...
                          sliver: SliverList(
                            delegate: SliverChildBuilderDelegate((context, index) {
                              ...
                              return DocAppointmentCard(
                                ...
                                onTap: () => context.read<DoctorDashboardBloc>().add(DocAndAppPatientDetailsNavEvent()),
  ...
  Widget _buildMetricsGrid(BuildContext context, DoctorDashboardLoaded state, Color primaryColor, bool isTab) {
    final List<Widget> metricCards = [
      DocMetricCard(title: 'Today', value: '${state.todaysAppointments.length}', subtext: '0 completed', icon: Icons.calendar_today_outlined, iconColor: primaryColor, isTab: isTab),
      DocMetricCard(title: 'Patients', value: '4', subtext: '0 new this week', icon: Icons.person_outline_rounded, iconColor: primaryColor, isTab: isTab),
      DocMetricCard(title: 'Done', value: '0', subtext: '0 follow-ups', icon: Icons.check_circle_outline_rounded, iconColor: Colors.teal, isTab: isTab),
      DocMetricCard(title: 'Stats', value: '0', subtext: '0 new patients', icon: Icons.analytics_outlined, iconColor: primaryColor, isTab: isTab),
    ];
    if (isTab) {
      return Row(
        children: List.generate(metricCards.length, (index) {
          final bool isLast = index == metricCards.length - 1;
          return Expanded(child: Padding(padding: EdgeInsets.only(right: isLast ? 0.0 : fieldSpace), child: metricCards[index]));
        }),
      );
    }
    return GridView.count(crossAxisCount: 2, shrinkWrap: true, padding: EdgeInsets.zero, physics: const NeverScrollableScrollPhysics(), mainAxisSpacing: fieldSpace, crossAxisSpacing: fieldSpace, childAspectRatio: 1.5, children: metricCards);
  }
  ...
  Widget _buildEmptyState(BuildContext context, String message) {
    return Container(
      ...
      child: Center(child: Text(message, style: TextStyle(fontFamily: appPoppinFont, fontSize: displayWidth(context) * 0.034, color: Colors.grey[500], fontWeight: FontWeight.w500))),
    );
  }

  Widget _buildWelcomeCard(BuildContext context, bool isDark, Color primaryColor, bool isTab) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: isDark ? Colors.black.withOpacity(0.2) : Colors.black.withOpacity(0.03), blurRadius: 15, offset: const Offset(0, 4))],
      ),
          ...
          children: [
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('WELCOME BACK', style: TextStyle(fontFamily: appPoppinFont, fontSize: isTab ? textWidth * 0.014 : textWidth * 0.028, fontWeight: FontWeight.bold, color: primaryColor, letterSpacing: 0.5)),
                  const SizedBox(height: 4.0),
                  Text('Dr. Rajesh Nagalingam', style: TextStyle(fontFamily: appPoppinFont, fontSize: isTab ? textWidth * 0.018 : textWidth * 0.044, fontWeight: FontWeight.w700, color: isDark ? Colors.white : const Color(0xFF1E293B), letterSpacing: -0.3)),
                  const SizedBox(height: 4.0),
                  Text('Dentist', style: TextStyle(fontFamily: appPoppinFont, fontSize: isTab ? textWidth * 0.015 : textWidth * 0.034, fontWeight: FontWeight.w600, color: isDark ? Colors.grey[400] : Colors.grey[700])),
                  const SizedBox(height: 8.0),
                  Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      ...
                      Expanded(child: Text('Ocimum dental clinic, Journalist colony, Hyd 500034', style: TextStyle(fontFamily: appPoppinFont, fontSize: isTab ? textWidth * 0.015 : textWidth * 0.03, color: Colors.grey[500]))),
                    ],
                  ),
                ],
              ),
            ...
            Container(
              ...
              decoration: BoxDecoration(color: isDark ? Colors.grey[800]!.withOpacity(0.4) : primaryColor.withOpacity(0.06), shape: BoxShape.circle),
  ...
  Widget _buildSectionHeader(BuildContext context, String title, String actionText, bool isDark, Color primaryColor, VoidCallback onTap, bool isTab) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        Text(title, style: TextStyle(fontFamily: appPoppinFont, fontSize: isTab ? displayWidth(context) * 0.02 : displayWidth(context) * 0.038, fontWeight: FontWeight.w700, color: isDark ? Colors.white : const Color(0xFF1E293B), letterSpacing: -0.3)),
        TextButton(style: TextButton.styleFrom(padding: EdgeInsets.zero, minimumSize: Size.zero, tapTargetSize: MaterialTapTargetSize.shrinkWrap), onPressed: onTap, child: Text(actionText, style: TextStyle(fontFamily: appPoppinFont, fontSize: isTab ? displayWidth(context) * 0.018 : displayWidth(context) * 0.032, fontWeight: FontWeight.w600, color: primaryColor))),
      ],
    );
  }

  Widget _buildChartCard(BuildContext context, String title, String badgeText, bool isDark, Widget chartContent, bool isTab) {
    return Container(
      padding: const EdgeInsets.all(16.0),
      decoration: BoxDecoration(
        ...
        border: Border.all(width: 0.5, color: Colors.grey.withOpacity(0.2)),
        boxShadow: [BoxShadow(color: isDark ? Colors.black.withOpacity(0.1) : Colors.black.withOpacity(0.02), blurRadius: 10, offset: const Offset(0, 2))],
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text(title, style: TextStyle(fontFamily: appPoppinFont, fontSize: isTab ? displayWidth(context) * 0.02 : displayWidth(context) * 0.036, fontWeight: FontWeight.w700, color: isDark ? Colors.white : const Color(0xFF1E293B))),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                decoration: BoxDecoration(color: isDark ? Colors.grey[800]!.withOpacity(0.5) : const Color(0xFFF1F5F9), borderRadius: BorderRadius.circular(6)),
                child: Text(badgeText, style: TextStyle(fontFamily: appPoppinFont, fontSize: isTab ? displayWidth(context) * 0.014 : displayWidth(context) * 0.026, color: isDark ? Colors.grey[400] : const Color(0xFF64748B), fontWeight: FontWeight.w500)),
              ),
            ],
          ),
  ...
  Widget _buildWeeklyChartBarList(BuildContext context, Color primaryColor, double height) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final List<Color> beautifulWeeklyPalette = [primaryColor, isDark ? const Color(0xFF2DD4BF) : const Color(0xFF0D9488), isDark ? const Color(0xFF34D399) : const Color(0xFF059669), isDark ? const Color(0xFF6EE7B7) : const Color(0xFF34D399), isDark ? const Color(0xFF059669) : const Color(0xFF14532D), isDark ? const Color(0xFF14B8A6) : const Color(0xFF0F766E), isDark ? const Color(0xFF4ADE80) : const Color(0xFF16A34A)];
    return CustomBarChart(values: const [6, 11, 1, 4, 9, 5, 2], labels: const ['MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT', 'SUN'], maxY: 12, barWidth: 14, chartHeight: height, barColors: beautifulWeeklyPalette, fontFamily: appPoppinFont, tooltipSuffix: 'Appointments', isTab: isTablet(context));
  }
  ...
  Widget _buildMonthlyChartBarList(BuildContext context, Color primaryColor, double height) {
    ...
    return CustomBarChart(isTab: isTablet(context), monthly: true, values: const [35, 48, 55, 68, 15, 22, 44, 62, 52, 30, 38, 18], labels: const ['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC'], maxY: 75, barWidth: 8, chartHeight: height, barColors: beautifulMonthlyColors, borderRadius: const BorderRadius.vertical(top: Radius.circular(4)), tooltipSuffix: 'Patients', fontFamily: appPoppinFont);
  }
}